...
class AffineWholeHead(Affine):
    ...
    def registerAtlasWholeHead(self,
        ...
        imageToImageTransformMatrix, worldToWorldTransformMatrix, optimizationSummary = \
            self.registerMeshToImageWholeHead(worldToWorldTransformMatrix=worldToWorldTransformMatrix,
                                     Ks=Ks,
    ...
    def optimizeTransformationWholeHead(self, 
    ...
    def registerMeshToImageWholeHead( self, worldToWorldTransformMatrix, Ks, maximalDeformationStopCriterion,
        ...
        if worldToWorldTransformMatrix is not None:
            ...
            imageToImageTransformMatrix = self.getInitializationWholeHead(initializationOptions, noneck)


            # Optimze image-to-image transform (template->subject)
            for K in Ks:
                imageToImageTransformMatrix, optimizationSummary = \
                    self.optimizeTransformationWholeHead( imageToImageTransformMatrix,
    ...
    def saveResultsWholeHead(self, image_name, template_name,
    ...
    def adjust_neck(self, T1, transformed_template_file_name, mesh_level1,
                    mesh_level2, neck_bounds, neck_tissues, visualizer, debug=False,
