...
class OnlineFEM:
    ...
    def __init__(
        ...
        if self.method == "TMS":
            ...
            assert (
                self.mesh.nodes.nr == node_nr_before
            ), "The mesh nodes changed when removing triangles! Please make sure the mesh is valid."
            assert (
                self.mesh.nodes.node_coord == nodes_before
            ).all(), "The mesh nodes changed when removing triangles! Please make sure the mesh is valid."
...
class FemTargetPointCloud:
    ...
    def __init__(
        ...
        # ensure that the nodes did not change
        assert (
            mesh_cropped.nodes.nr == mesh.nodes.nr
        ), "The mesh nodes changed when removing triangles! Please make sure the mesh is valid."
        assert (
            mesh_cropped.nodes.node_coord == mesh.nodes.node_coord
        ).all(), "The mesh nodes changed when removing triangles! Please make sure the mesh is valid."
