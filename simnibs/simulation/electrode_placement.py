...
def _get_roi(center, radius, mesh, mesh_surface=[5, 1005], min_cos=0.1):
    ...
    # determine average edge length in ROI triangles by sampling
    tr_in_roi = np.any(np.isin(mesh.elm[mesh.elm.triangles], nodes_in_roi), axis=1)
    avg_l_roi = np.average(
        np.linalg.norm(
            mesh.nodes[mesh.elm[mesh.elm.triangles, 0][tr_in_roi]]
            - mesh.nodes[mesh.elm[mesh.elm.triangles, 1][tr_in_roi]],
            axis=1,
        )
    )
    nodes_in_roi_ext = nodes_in_surface[
        (distances <= radius + 2 * avg_l_roi) * (center_normal.dot(normals.T) > min_cos)
    ]
...
def put_electrode_on_mesh(
    ...
    if elec.dimensions_sponge is not None and len(elec.dimensions_sponge) > 0:
        ...
        if elec.shape not in ["rect", "ellipse"]:
            raise NotImplementedError(
                "Sponges are only implemented for rectangular"
