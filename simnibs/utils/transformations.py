...
def transform_tdcs_positions(coords, transf_type, transf_def, pos_y=None, mesh=None, surface_tags=None):
    ...
    mesh: simnibs.msh.mesh_io.Msh
        Mesh structure
    surface_tags: int | list[int] | tuple | np.ndarray
        Surface tags to project points onto
    Returns
    ------
    ...
    if surface_tags is None:
        surface_tags = ElementTags.SCALP_TH_SURFACE

    if mesh is not None:
        coords_transf = mesh.project_points_on_surface(coords_transf, surface_tags=surface_tags)
        if pos_y is not None:
            pos_y_transf = mesh.project_points_on_surface(
                pos_y_transf, surface_tags=surface_tags
            )
    ...
    if transf_type == "affine":
        ...
        if mesh is not None:
            coords_transf = mesh.project_points_on_surface(
                coords_transf, surface_tags=surface_tags, distance=distances
            )
