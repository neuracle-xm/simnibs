...
def _current_tick_bounds(settings: GASettings) -> tuple[int, int]:
    ...
    if not np.all(
        np.isfinite(
            [settings.current_step_ma, settings.current_min_ma, settings.current_max_ma]
        )
    ):
        raise ValueError("电流边界和步长必须是有限数字")
...
def _require_genetic_algorithm() -> Any:
    ...
    if ga is None:
        raise RuntimeError(
            "缺少算法依赖 geneticalgorithm==1.0.2，请先在 simnibs_env 中安装"
...
def run_genetic_optimization(
    ...
    if any(
        not np.isfinite(probability) or probability < 0 or probability > 1
