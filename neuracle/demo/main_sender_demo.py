...
使用 SenderThread 发送测试消息到 backend_to_simnibs 队列，
并监听 simnibs_to_backend 队列接收进度和结果。

启动方式: python -m neuracle.demo.main_sender_demo
"""
...
logger = logging.getLogger("neuracle.demo.main_sender_demo")
...
def run_listener_thread(
    ...
    retry_delay: int = 5,
) -> tuple[RabbitMQListener, threading.Thread]:
    """启动监听线程"""
    ...
    while True:
        ...
        try:
            ...
            if message.get("id") == task_id:
                ...
                if progress_rate == 0 and status_message not in ("", "任务开始"):
...
def send_message_and_wait(
    ...
    # 启动监听线程
    listener, listener_thread = run_listener_thread(
        host,
        port,
        recv_queue,
        message_queue,
...
def main():
    """主函数"""
    setup_logging(os.path.join(PROJECT_ROOT, "log"))
    load_env()
    ...
    # main.py 监听 listen_queue_name 接收任务，发送结果到 send_queue_name
    # 所以 demo 发送任务到 listen_queue_name，接收结果从 send_queue_name
