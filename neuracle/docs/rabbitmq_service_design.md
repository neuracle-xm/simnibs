...
本文档描述 `python -m neuracle.main` 当前代码已经实现的服务结构，而不是最初的规划稿。
...
listen_queue_name
  |
  v
RabbitMQListener.consume_forever()
  |
  v
handle_message()
  |
...
### 3.2 `RabbitMQListener`

位于 [`neuracle/rabbitmq/listener.py`](../../neuracle/rabbitmq/listener.py)。
...
- `handle_message()` 无论成功失败都会在 `finally` 中 `basic_ack`
...
位于 [`neuracle/rabbitmq/sender_thread.py`](../../neuracle/rabbitmq/sender_thread.py)。
...
5. `run_service()` 内部先启动 `SenderThread`
6. 再创建 `RabbitMQListener`
...
- `handle_message()` 负责解析和校验阶段异常
- `execute_task()` 负责具体任务执行异常
...
- 目标函数固定为 `focality`
- `atlas` ROI 尚未真正接入 atlas 脑区解析，当前仍是占位实现
- `current_A`、`current_B` 仅参与请求校验，不直接传给优化器
- 若未传 `electrode_pair1_center` 等可选字段，则使用 `ti_optimize.py` 中的默认值
...
配置文件来自 `neuracle/.env`，由 `load_env()` 加载。
...
- [`neuracle/demo/main_sender_demo.py`](../../neuracle/demo/main_sender_demo.py)
- [`neuracle/demo/backend_side_demo.py`](../../neuracle/demo/backend_side_demo.py)
- [`neuracle/demo/rabbitmq_example.py`](../../neuracle/demo/rabbitmq_example.py)

其中 `main_sender_demo.py` 最接近 `main.py` 当前接口，适合做端到端联调。
