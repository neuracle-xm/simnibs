...
def run_segmentation(
    ...
    if isinstance(num_threads, int) and num_threads > 0:
        ...
        logger.info("线程数无效 (%s)，使用 MAX_THREADS (%d)", num_threads, MAX_THREADS)
    show_figs = False
    show_movies = False
...
def parse_arguments(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m neuracle.charm.segment",
        description="Run segmentation for CHARM pipeline",
    )
...
def main() -> None:
    ...
    run_segmentation(subject_dir, debug=args.debug)
