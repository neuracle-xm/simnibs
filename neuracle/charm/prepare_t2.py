...
def parse_arguments(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m neuracle.charm.prepare_t2",
        description="Prepare and optionally register T2 image for CHARM pipeline",
    )
    parser.add_argument('subid', help='Subject ID (e.g., sub001)')
    parser.add_argument('t2', help='Path to T2 NIfTI file')
    parser.add_argument(
        '--register-t2',
        action='store_true',
        help='Register T2 to T1 space',
    )
    parser.add_argument(
        '--forceqform',
        action='store_true',
